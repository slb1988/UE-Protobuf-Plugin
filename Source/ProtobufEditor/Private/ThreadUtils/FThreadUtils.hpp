...
class FProtoThread : public FRunnable
{
public:
	using FCallback = TFunction<void()>;
	explicit FProtoThread(const TCHAR *InThreadName, const FCallback& InRunFunc)
...
private:
    FProtoThread(const FProtoThread&) = delete;
    FProtoThread& operator=(const FProtoThread&) = delete;
